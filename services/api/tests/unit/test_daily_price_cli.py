...
def test_daily_price_cli_is_independent_and_uses_safe_summary(
    ...
    monkeypatch.setattr(
        ...
        lambda **_kwargs: SimpleNamespace(
            capture=lambda _start, _end, _symbols=None: report
        ),
    )
...
def test_daily_price_cli_can_bound_a_backfill_to_one_stored_symbol(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    ...
    monkeypatch.setattr(
        command,
        "AlpacaProfitRatioCaptureProvider",
        lambda **kwargs: SimpleNamespace(close=lambda: None, ids=kwargs["instrument_ids"]),
    )
    report = DailyPriceCaptureReport(
        date(2026, 9, 4), date(2026, 9, 4), "COMPLETE", 1, 1, 0, 1, 0, 0, ()
    )
    monkeypatch.setattr(
        command,
        "DailyPriceCaptureService",
        lambda **_kwargs: SimpleNamespace(
            capture=lambda start, end, symbols: captured.append((start, end, symbols)) or report
        ),
    )
    assert command.main(
        ["--start", "2026-09-04", "--end", "2026-09-04", "--symbols", "SNDK"]
    ) == 0
    assert captured == [(date(2026, 9, 4), date(2026, 9, 4), ("SNDK",))]
